...
public class Cuboid implements Iterable<Block>, Cloneable, ConfigurationSerializable {
	...
	public Cuboid(Location l1, Location l2) {
		if (! l1.getWorld().equals(l2.getWorld())) {
	       ...
	       int xonetemp = l1.getBlockX();
	        int yonetemp = l1.getBlockY();
	        int zonetemp = l1.getBlockZ();
	        int xtwotemp = l2.getBlockX();
	        int ytwotemp = l2.getBlockY();
	        int ztwotemp = l2.getBlockZ();
	        
	        worldName = l1.getWorld().getName();
	        x1 = Math.min(xonetemp, xtwotemp);
	        y1 = Math.min(yonetemp, ytwotemp);
	        z1 = Math.min(zonetemp, ztwotemp);
	        x2 = Math.max(xonetemp, xtwotemp);
	        y2 = Math.max(yonetemp, ytwotemp);
	        z2 = Math.max(zonetemp, ztwotemp);
	        // make sure the graves fit
	        x2 = x2 - ((x2-x1) % 2); 
	        z2 = z2 - ((z2-z1) % 2);
	...
	private Cuboid(String CuboidString) {
		this.worldName = worldName;
		this.x1 = Math.min(x1, x2);
		this.x2 = Math.max(x1, x2);
		this.y1 = Math.min(y1, y2);
	...
	public World getWorld() {
		...
		if (world == null) {
			throw new IllegalStateException(ChatColor.RED + "world '" + worldName + "' is not loaded" + ChatColor.WHITE);
		}
		return world;
	}
	...
	public Cuboid enlarge(int amount) {
		Location l1 = this.getLowerNE().subtract(amount, 0, amount);
		Location l2 = this.getUpperSW().add(amount, 0, amount);
	...
	public void fill(int blockId, byte data) {
		...
		MessageManager.getInstance().debugMessage("Cuboid fill: " + this + ": set " + blockId + "/" + data + ": " + (System.nanoTime() - start) + "ns");
	}

	public void lowerFill(int blockId, byte data) {
		long start = System.nanoTime();
		...
		MessageManager.getInstance().debugMessage("Cuboid lowerFill: " + this + ": set " + blockId + "/" + data + ": " + (System.nanoTime() - start) + "ns");
	}
	...
	public String toVecString(){
		return "_" + worldName + "_" + x1 + "" + y1 + "" + z1 + "_" + x2 + "" + y2 + "" + z2;
	}
	/**
 ...
 */
	public void SetMetaData(String id, String value) {
		long start = System.nanoTime();
		MetadataUtil m = new MetadataUtil(GraveYardMain.getInstance());
		int c = 0;
		for (Block b : this) {
			if(b.getType() == Material.AIR){
				b = b.getRelative(BlockFace.DOWN);
			}
			m.getInstance().set(b ,id, value);
			c++;
//			b.setMetadata(id, new FixedMetadataValue(GraveYardMain.getInstance(), value));
		}
		MessageManager.getInstance().debugMessage("Cuboid SetMetaData: " + this + ": set " + c + " blocks to " + id + " with owner " + value + ": " + (System.nanoTime() - start) + "ns");
	}
	...
	public void fillEdges(int blockId, byte data) {
		long start = System.nanoTime();

		double startX = this.getLowerX();
		double startY = this.getLowerY();
		double startZ = this.getLowerZ();
		MessageManager.getInstance().debugMessage("x size" + (this.getSizeX()-1));
		MessageManager.getInstance().debugMessage("z size" + (this.getSizeZ()-1));

		for (Block b : this){
			int x = b.getX()-this.getLowerX();
			int y = b.getY()-this.getLowerY();
			int z = b.getZ()-this.getLowerZ();
			if ((x >= 0 && z == 0) || 
					(x >= 0 && z >= (this.getSizeZ()-1)) ||
					( x == 0 && z >= 0) || 
					(x >= (this.getSizeX()-1) && z >= 0)){
			    if (data != 0){
			        b.setTypeIdAndData(blockId, data, false);
				MessageManager.getInstance().debugMessage("x " + x + " z " + z);
			    }else{
				MessageManager.getInstance().debugMessage("x " + x + " z " + z);
			        b.setTypeId(blockId);
			    }
		...
		MessageManager.getInstance().debugMessage("Cuboid fillEdges: " + this + ": set " + blockId + "/" + data + ": " + (System.nanoTime() - start) + "ns");
		}
	
		public List<Block> getEdges(){
			long start = System.nanoTime();
			...
			MessageManager.getInstance().debugMessage("Cuboid getEdges: " + this + ": return edge blocks : " + (System.nanoTime() - start) + "ns");
			return bl;
		}

	public void setPreview(int blockId, byte data) {
			long start = System.nanoTime();
	...
	public void resetPreview(int blockId, byte data) {
		long start = System.nanoTime();
		...
		for (Block b : this){
			...
			if ((x >= 0 && z == 0) || 
					...
					( x == 0 && z >= 0) || 
					(x >= (this.getSizeX()-1) && z >= 0)){
				if( b.getTypeId()!= blockId && b.getData()!= data){
					continue;
				}
